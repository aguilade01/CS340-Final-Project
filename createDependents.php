...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    } elseif (!filter_var($Dname, FILTER_VALIDATE_REGEXP, ["options" => ["regexp" => "/^[a-zA-Z\s]+$/"]])) {
    ...
    } elseif (!filter_var($Relationship, FILTER_VALIDATE_REGEXP, ["options" => ["regexp" => "/^[a-zA-Z\s]+$/"]])) {
    ...
    if (empty($Dname_err) && empty($Relationship_err) && empty($Sex_err) && empty($Bdate_err)) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            mysqli_stmt_bind_param($stmt, "sssss", $param_Ssn, $param_Dname, $param_Sex, $param_Bdate, $param_Relationship);
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header">
                        ...
                        <h3>For employee with SSN = <?php echo htmlspecialchars($Ssn); ?></h3>
                    </div>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="form-group <?php echo (!empty($Dname_err)) ? 'has-error' : ''; ?>">
                            <label>Dependent's Name</label>
                            <input type="text" name="Dname" class="form-control" value="<?php echo htmlspecialchars($Dname); ?>">
                            <span class="help-block"><?php echo $Dname_err; ?></span>
                        </div>
                        <div class="form-group <?php echo (!empty($Relationship_err)) ? 'has-error' : ''; ?>">
                            <label>Relationship</label>
                            <input type="text" name="Relationship" class="form-control" value="<?php echo htmlspecialchars($Relationship); ?>">
                            <span class="help-block"><?php echo $Relationship_err; ?></span>
                        </div>
                        <div class="form-group <?php echo (!empty($Sex_err)) ? 'has-error' : ''; ?>">
                            <label>Sex</label>
                            <input type="text" name="Sex" class="form-control" value="<?php echo htmlspecialchars($Sex); ?>">
                            <span class="help-block"><?php echo $Sex_err; ?></span>
                        </div>
                        <div class="form-group <?php echo (!empty($Bdate_err)) ? 'has-error' : ''; ?>">
                            <label>Birth date</label>
                            <input type="date" name="Bdate" class="form-control" value="<?php echo htmlspecialchars($Bdate); ?>">
